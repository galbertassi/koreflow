...
import { Bell, Calendar as CalendarIcon, ChevronDown, Clock, ChevronLeft, ChevronRight, PlusCircle, Printer, Download, Menu } from "lucide-react";
...
export function Header() {
  ...
  const { planejamentos, configuracoes, toggleSidebar } = useStore();
  ...
  return (
    <header className={`print:hidden h-24 px-8 flex items-center ${pathname === '/' ? 'justify-between' : 'justify-end'} border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-10`}>
      {pathname === '/' && (
        <div className="flex items-center gap-3">
          <button onClick={toggleSidebar} className="md:hidden p-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors text-sidebar-foreground">
            <Menu className="w-5 h-5" />
          </button>
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Bem-vindo(a), {nomeUsuario}.</h2>
            <p className="text-muted-foreground text-sm mt-1 hidden sm:block">Planejamento, execução e entregas centralizados em um único fluxo.</p>
          </div>
        </div>
      )}
      ...
      {pathname !== '/' && (
        <div className="md:hidden absolute left-4">
          <button onClick={toggleSidebar} className="p-2 rounded-lg bg-secondary/50 hover:bg-secondary transition-colors text-sidebar-foreground">
            <Menu className="w-5 h-5" />
          </button>
        </div>
      )}
