...
function CreateExecutionModal() {
  ...
  const handleSubmit = () => {
    ...
    addExecucao({ titulo, categoria: categoria.trim() || "Planejamento", entrega: "", prioridade, tipoPlanejamento: tipoPlanejamento as any });
    setTitulo("");
    setCategoria("");
    setPrioridade("Media");
...
function EditExecutionModal() {
  const { closeModal, data } = useModal();
  const { execucoes, updateExecucao } = useStore();
  
  const execucaoId = data?.execucaoId as string;
  const execucao = execucoes.find(e => e.id === execucaoId);

  const [titulo, setTitulo] = useState(execucao?.titulo || "");
  const [categoria, setCategoria] = useState(execucao?.categoria || "");
  const [prioridade, setPrioridade] = useState(execucao?.prioridade || "Media");
  const [tipoPlanejamento, setTipoPlanejamento] = useState(execucao?.tipoPlanejamento || "Previsto");

  const handleSubmit = () => {
    if (!titulo.trim() || !execucaoId) return;
    updateExecucao(execucaoId, { 
      titulo, 
      categoria: categoria.trim() || "Planejamento", 
      prioridade, 
      tipoPlanejamento: tipoPlanejamento as any 
    });
    closeModal();
  };
    ...
    <Dialog open onOpenChange={(open) => !open && closeModal()}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>Edite os detalhes desta execucao.</DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4 py-2">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="edit-exec-title">Titulo *</Label>
            <Input id="edit-exec-title" placeholder="Ex: Relatorio de performance de maio" value={titulo} onChange={(e) => setTitulo(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="edit-exec-category">Categoria</Label>
            <Input id="edit-exec-category" placeholder="Ex: Relatorios, Campanhas..." value={categoria} onChange={(e) => setCategoria(e.target.value)} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="edit-exec-tipo">Planejamento</Label>
              <select id="edit-exec-tipo" value={tipoPlanejamento} onChange={(e) => setTipoPlanejamento(e.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs focus:outline-none focus:ring-2 focus:ring-ring/20">
                <option value="Previsto">Previsto</option>
                <option value="Demanda Extra">Demanda Extra</option>
              </select>
            </div>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="edit-exec-priority">Prioridade</Label>
              <select id="edit-exec-priority" value={prioridade} onChange={(e) => setPrioridade(e.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs focus:outline-none focus:ring-2 focus:ring-ring/20">
                <option value="Alta">Alta</option>
                <option value="Media">Media</option>
                <option value="Baixa">Baixa</option>
              </select>
...
export function ModalManager() {
  ...
  return (
    <>
      ...
      {type === "EDIT_EXECUTION" && <EditExecutionModal />}
