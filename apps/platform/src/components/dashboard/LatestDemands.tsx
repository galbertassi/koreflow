...
export function LatestDemands() {
  ...
  const tableData = demands.map(e => {
    return {
      ...
      prioridade: e.priority === "URGENT" ? "Urgente" : e.priority === "HIGH" ? "Alta" : e.priority === "MEDIUM" ? "Média" : "Baixa",
              ...
              <tr key={idx} className="border-b border-border/40 hover:bg-secondary/20 transition-colors">
                ...
                <td className="py-4 px-4">
                  <div className="flex items-center gap-2">
                    {configuracoes?.foto ? (
                      <img src={configuracoes.foto} alt="Você" className="w-6 h-6 rounded-full object-cover" />
                    ) : (
