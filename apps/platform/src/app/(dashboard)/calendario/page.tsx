...
function CalendarioContent() {
  ...
  const handleAdd = () => {
    ...
    addEvento(novoEvento as any);
    setNovoEvento({ titulo: "", data: format(new Date(), "yyyy-MM-dd"), hora: "", tipo: "lembrete", notificacao: true, alarme: true });
    setShowForm(false);
  };
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="flex-1 bg-white rounded-2xl border border-border/50 overflow-x-auto flex flex-col">
    ...
    {(e as any).alarme && <Clock className="w-2.5 h-2.5 text-[#8B5CF6]/60" />}
    {(e as any).notificacao && <Bell className="w-2.5 h-2.5 text-[#8B5CF6]/60" />}
  </div>
...
export default function CalendarioPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted-foreground">Carregando calendário...</div>}>
      <CalendarioContent />
    </Suspense>
  );
}
