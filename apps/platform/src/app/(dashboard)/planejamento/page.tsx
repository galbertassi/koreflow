  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <button onClick={() => openModal("CREATE_PLANNING")} className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2.5 bg-[#8B5CF6] hover:bg-[#7C3AED] text-white rounded-xl text-sm font-medium transition-colors shadow-sm">
          <Plus className="w-4 h-4" /> Novo Ciclo
        </button>
      </div>

      <div className="flex items-center gap-1 mb-6 bg-white rounded-xl border border-border/50 p-1 w-full sm:w-fit overflow-x-auto hide-scrollbar whitespace-nowrap">
        {viewTabs.map((tab) => (
          <button key={tab} onClick={() => setView(tab)} className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors shrink-0 flex items-center gap-2 ${view === tab ? "bg-[#8B5CF6]/10 text-[#8B5CF6]" : "text-muted-foreground hover:text-foreground"}`}>
