...
*Dados lidos diretamente da sua agenda no KORE FLOW.*`, actions: [] };
  ...
  if (t.includes("priorizar") || t.includes("prioridade")) {
    const altasPrioridades = execucoes.filter(e => e.prioridade === "Alta" && e.status !== "Concluida");
  ...
  if (t.includes("relatório") || t.includes("resumo") || t.includes("produtividade geral")) {
    const totalConcluidas = execucoes.filter(e => e.status === "Concluida").length;
    const totalPendentes = execucoes.filter(e => e.status !== "Concluida").length;
  ...
  if (t.includes("pendentes") || t.includes("auditar")) {
    const pendentes = execucoes.filter(e => e.status !== "Concluida");
